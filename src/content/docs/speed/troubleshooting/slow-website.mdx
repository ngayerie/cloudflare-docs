...
Before diving into detailed debugging, check if your actual visitors are experiencing performance issues. [Web Analytics](/web-analytics/) provides RUM data that shows real-world performance metrics from your visitors.

1. Go to the Cloudflare dashboard.

   <DashButton url="/?to=/:account/:zone/web-analytics" />
2. Review the **Core Web Vitals** section.
3. Look for patterns in slow performance (specific pages, geographic regions, or device types).

RUM data helps you understand whether the slowness is widespread or isolated to specific conditions. If RUM shows good performance but you are experiencing issues locally, the problem may be with your network or device rather than the website itself.
...
## Step 2: Identify slow assets

If Observatory and RUM data point to specific issues, use browser developer tools to investigate individual assets.
...
```bash
sudo mtr -rwz www.example.com
```

:::note
On macOS, `mtr` requires `sudo` to send ICMP packets. You can install it via [Homebrew](https://formulae.brew.sh/formula/mtr): `brew install mtr`.
:::

The `-z` flag displays the AS number for each hop, helping identify which network is responsible for latency or packet loss.
...
- [ ] Verify traffic is going through Cloudflare (check for `cf-ray` header)
- [ ] Run Observatory tests and review Core Web Vitals
- [ ] Check RUM data to see if real users are experiencing issues
